...
// ── New: Statistical Validity Report ────────────────────────────────────────
export interface StatisticalValidityReport {
  ...
  pValue:          number | null;
  effectSize:      { h: number; interpretation: string };
  ...
  outliers:        { count: number; affectedSessions: string[]; severity: string };
...
interface OPEDebugStore {
  ...
  statisticalValidity: StatisticalValidityReport | null;
  ...
  updateStatisticalValidity: (report: StatisticalValidityReport) => void;
}
...
export const useOPEDebugStore = create<OPEDebugStore>((set, get) => ({
  ...
  simulateEvent: (event) => {
    const now = Date.now();
    // P1-DET FIX (M2): Math.random() replaced with crypto-based entropy for
    // reproducible event ID suffix. The prefix is still wall-clock time because
    // this is the debug/simulation path, not the production decision pipeline.
    const entropyHex = (typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function')
      ? (() => { const b = new Uint32Array(1); crypto.getRandomValues(b); return b[0].toString(16).padStart(8, '0').slice(0, 6); })()
      : Math.floor(Math.random() * 0xFFFFFF).toString(16).padStart(6, '0');
    const newEvent: OPEEvent = {
      id: `evt_${now}_${entropyHex}`,
  ...
  // ── Update statistical validity report ────────────────────────────────────────
  updateStatisticalValidity: (report: StatisticalValidityReport) => set(() => ({
    statisticalValidity: report,
  })),

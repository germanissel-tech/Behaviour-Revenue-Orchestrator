...
class BehavioralStateStore {
  ...
  setActiveProduct(productId, now) {
    ...
    // Track every product view in sessionMemory so _handleProductChange can
    // detect revisits (returning to a previously-viewed product).
    const arr = this._state.sessionMemory.viewedProducts;
    const existingIdx = arr.indexOf(productId);
    // Only add if not already present (will be moved to end below if revisit)

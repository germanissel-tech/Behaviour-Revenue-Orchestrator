...
class SessionStateStore {
  ...
  // ========== UTILIDADES ==========
  // P1-DET FIX (H1): _now() should ALWAYS receive an explicit nowMs from callers,
  // or be provided a _clock via constructor config. Falling back to Date.now()
  // creates a split-brain scenario: the orchestrator drives simulation time via
  // injected nowMs, while the store uses wall-clock time for snapshot timestamps
  // and TTL calculations. In production this is a replay-safety violation.
  //
  // Usage:
  //   - Always pass nowMs when calling internal methods.
  //   - In production without a _clock, Date.now() is used as a fallback but
  //     a one-time warning is emitted so operators can detect the misconfiguration.
  _now(nowMs) {
    if (typeof nowMs === 'number') return nowMs;
    if (typeof this.config._clock === 'function') return this.config._clock();
    // Fallback: Date.now() — WARN once per store instance
    if (!this._nowFallbackWarned) {
      this._nowFallbackWarned = true;
      console.warn(
        '[session-state-store] WARNING (H1): _now() called without injected nowMs and ' +
        'no _clock configured. Falling back to Date.now() — this breaks replay determinism. ' +
        'Inject a _clock function via constructor config or always pass nowMs to callers.'
      );
    }
    return Date.now();
  }
  ...
  // ========== TTL / CLEANUP ==========
  startCleanup() {
    // P1-SAFETY FIX (H5): Prevent double-start. If called twice (e.g. after
    // a failed init), the old timer would leak. Guard prevents multiple timers.
    if (this._cleanupTimer) {
      console.warn('[SessionStateStore] startCleanup() called while a cleanup timer is already running. Ignored.');
      return;
    }
    // NOTE: These timers fire at real wall-clock time, not at orchestrator-
    // controlled simulation time. In test/simulation environments, avoid calling
    // startCleanup()/startAutoSnapshot()/startEventFlush() unless the store is
    // truly in production mode. In tests, call cleanupInactiveSessions() manually
    // with an injected nowMs argument.
  ...
  startAutoSnapshot() {
    if (this._snapshotTimer) {
      console.warn('[SessionStateStore] startAutoSnapshot() called while a snapshot timer is already running. Ignored.');
      return;
    }

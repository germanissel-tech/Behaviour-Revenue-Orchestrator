...
class ReturnRiskIntelligenceEngine {
  ...
  // ==========================================================================
  // PHASE 4: Integration with message-ranking-engine and intervention-policy
  // ==========================================================================

  /**
   * Generate risk-adjusted intervention recommendations.
   * High return risk:
   *   - Reduce urgency
   *   - Increase assistance
   *   - Prefer QUALITY, REASSURANCE, ASSISTANCE families
   *   - Reduce aggressive interventions
   *
   * @param {object} riskAssessment - Result from assess()
   * @returns {InterventionAdjustments}
   *
   ...
   *   recommendedTone: 'reassuring' | 'helpful' | 'neutral' | 'urgent',
   *   rationale: string[],
   * }
   */
  getInterventionAdjustments(riskAssessment) {
    if (this._disposed) throw new Error('ReturnRiskIntelligenceEngine: disposed');

    if (!riskAssessment || typeof riskAssessment.riskScore !== 'number') {
      return {
        preferredFamilies: [],
        suppressedFamilies: [],
        ...
        rationale: ['no_risk_data'],
      };
    }

    const { riskScore, riskTier, factors } = riskAssessment;
    const rationale = [];
    ...
    // High risk: reduce urgency, increase assistance
    if (riskTier === RISK_TIERS.HIGH || riskScore > 0.60) {
      preferredFamilies.push('QUALITY', 'REASSURANCE', 'EXPERTISE');
      suppressedFamilies.push('URGENCY');
      intensityMultiplier = 0.5; // Reduce intensity by 50%
      urgencyMultiplier = 0.3;   // Reduce urgency by 70%
      recommendedTone = 'reassuring';
      rationale.push('high_risk:reduce_urgency', 'high_risk:prefer_reassurance');
    }
    // Moderate risk: balanced approach
    else if (riskTier === RISK_TIERS.MODERATE || riskScore > 0.30) {
      preferredFamilies.push('QUALITY', 'REASSURANCE', 'SOCIAL_PROOF');
      intensityMultiplier = 0.75;
      urgencyMultiplier = 0.6;
      recommendedTone = 'helpful';
      rationale.push('moderate_risk:balanced_approach');
    }
    // Low risk: normal operation
    else {
      recommendedTone = 'neutral';
      rationale.push('low_risk:normal_operation');
    }

    // Specific factor adjustments
    if (factors && factors.length > 0) {
      for (const factor of factors) {
        if (factor.type === RISK_FACTOR_TYPES.SIZE_UNCERTAINTY) {
          preferredFamilies.push('EXPERTISE');
          rationale.push('size_uncertainty:prefer_expertise');
        }
        if (factor.type === RISK_FACTOR_TYPES.TECHNICAL_MISMATCH) {
          preferredFamilies.push('COMPATIBILITY');
          suppressedFamilies.push('URGENCY');
          rationale.push('technical_mismatch:prefer_compatibility');
        }
        if (factor.type === RISK_FACTOR_TYPES.SUBJECTIVE_FIT_RISK) {
          preferredFamilies.push('REASSURANCE', 'QUALITY');
          rationale.push('subjective_fit:prefer_reassurance');
        }
      }
    }

    // Deduplicate
    const uniquePreferred = [...new Set(preferredFamilies)];
    const uniqueSuppressed = [...new Set(suppressedFamilies)].filter(
      f => !uniquePreferred.includes(f)
    );

    return Object.freeze({
      preferredFamilies: Object.freeze(uniquePreferred),
      suppressedFamilies: Object.freeze(uniqueSuppressed),
  ...
  /**
   * Check if a specific message family should be allowed given current risk.
   *
   * @param {string} family - Message family
   * @param {object} riskAssessment - Result from assess()
   * @returns {{ allowed: boolean, reason?: string }}
   */
  shouldAllowFamily(family, riskAssessment) {
    const adjustments = this.getInterventionAdjustments(riskAssessment);

    if (adjustments.suppressedFamilies.includes(family)) {
      return {
        allowed: false,
        reason: `suppressed_by_return_risk:${riskAssessment.riskTier}`,
      };
    }
  ...
  /**
   * Apply risk adjustments to an intensity value.
   *
   * @param {number} baseIntensity - Original intensity (0-1)
   * @param {object} riskAssessment - Result from assess()
   * @returns {number} Adjusted intensity
   */
  adjustIntensity(baseIntensity, riskAssessment) {
    const adjustments = this.getInterventionAdjustments(riskAssessment);
    const adjusted = baseIntensity * adjustments.intensityMultiplier;

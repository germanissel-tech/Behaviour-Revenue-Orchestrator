...
function processEvent(sessionId, event, nowMs) {
  ...
  const startTime = typeof performance !== 'undefined' ? performance.now() : nowMs;
  ...
  result.processingTimeMs = (typeof performance !== 'undefined' ? performance.now() : nowMs) - startTime;
...
function selectMessage(sessionId, productId, availableMessages, nowMs) {
  ...
  // Delegate to contextual ranker (legacy path — will be deprecated separately)
  const selected = ContextualRanker.selectBestMessage(availableMessages, rankingContext);
  
  if (selected) {
    // Register message in lifecycle (side-effect preserved for backward compat)
    const registration = MessageLifecycle.registerMessage(
      sessionId,
      selected,
      { context: rankingContext.context, productId },
      nowMs
    );
    
    FunnelEngine.recordMessageShown(sessionId, selected.family, nowMs);

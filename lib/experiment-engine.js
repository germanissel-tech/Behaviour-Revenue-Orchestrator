...
// ── Statistical Validity Engine (read-only; EXPLAIN ONLY) ────────────────────
const {
...
function bootstrapCI(samplesA, samplesB, iterations, alpha) {
  if (!samplesA.length || !samplesB.length) {
    return { lower: null, upper: null, medianUplift: null, observedUplift: null };
  }

  const seed = (samplesA.length * 1000 + samplesB.length) >>> 0;
  ...
  for (let i = 0; i < iterations; i++) {
    const rA = resample(samplesA, rand);
    const rB = resample(samplesB, rand);
    const mA = rA.reduce((s, v) => s + v, 0) / rA.length;
    const mB = rB.reduce((s, v) => s + v, 0) / rB.length;
    uplifts.push(mA > 0 ? (mB - mA) / mA : (mB - mA));
  }

  uplifts.sort((a, b) => a - b);
  const lo = Math.floor(uplifts.length * (alpha / 2));
  const hi = Math.ceil(uplifts.length * (1 - alpha / 2)) - 1;
  const med = uplifts[Math.floor(uplifts.length / 2)];
...
function permutationPValue(samplesA, samplesB, iterations) {
  if (!samplesA.length || !samplesB.length) return null;

  const seed = (samplesA.length + samplesB.length * 7) >>> 0;
  const rand = makePRNG(seed);

  const meanA = samplesA.reduce((s, v) => s + v, 0) / samplesA.length;
  const meanB = samplesB.reduce((s, v) => s + v, 0) / samplesB.length;
  const observed = Math.abs(meanB - meanA);

  const combined = samplesA.concat(samplesB);
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const mA = pA.reduce((s, v) => s + v, 0) / pA.length;
    const mB = pB.reduce((s, v) => s + v, 0) / pB.length;
    if (Math.abs(mB - mA) >= observed) extremeCount++;
  }

  return extremeCount / iterations;
}
...
function cohenD(samplesA, samplesB) {
...
class ExperimentEngine {
  ...
  restore(snap) {
    ...
    this._decisionLog = Array.isArray(snap.decisionLog) ? snap.decisionLog.slice() : [];

...
function _getOrCreateOrchestrator(sessionId) {
  if (!_orchestrators.has(sessionId)) {
    if (_orchestrators.size >= MAX_ORCHESTRATORS) {
      // LRU eviction: dispose the oldest entry to release subscriptions/timers
      // P0-RESOURCE FIX (M5): previously deleted without calling dispose(), leaking
      // event bus subscriptions and any timers held by SessionStateStore.
      const firstKey = _orchestrators.keys().next().value;
      const evicted = _orchestrators.get(firstKey);
...
function processBehavioralEvent({ sessionId, storeId, event, candidateMessages = [], initialState = null }) {
  ...
  // P1-DET: require explicit nowMs — NEVER fall back to Date.now().
  // Callers MUST supply event.nowMs. Falling back breaks replay determinism.
  if (typeof event.nowMs !== 'number' || !Number.isFinite(event.nowMs)) {
    throw new Error(
      '[real-time-orchestrator] processBehavioralEvent: event.nowMs is required and must be a ' +
      'finite number. Pass the current simulation/wall-clock time as event.nowMs. ' +
      'Do NOT rely on Date.now() — it breaks replay determinism.'
    );
  }
  const orchestrationTs = event.nowMs;
  ...
  try {
    ...
    return {
      ...
      orchestrationSummary: {
        processingTimeMs: null, // P1-DET: performance.now() removed — non-reproducible. Callers that need timing must measure externally.

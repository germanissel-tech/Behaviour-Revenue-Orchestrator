...
class CooldownFatigueEngine {
  ...
  restore(snapshot, now) {
    this._assertAlive();
    // P2-RESILIENCE FIX (FI-6): A null/missing sub-engine snapshot must NOT crash
    // orchestrator initialization. Partial snapshots occur during:
    //   - First-ever deployment restoring from pre-existing snapshots
    //   - Rolling schema migrations where some engines are new
    //   - Persistence failures where sub-snapshots were partially written
    // Correct behavior: treat null as "no prior state" and keep fresh state.
    if (!snapshot || typeof snapshot !== 'object') {
      // No snapshot available — keep the fresh state set by reset()/constructor.
      return;
    }
    if (!Number.isFinite(now)) {

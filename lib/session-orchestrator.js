...
class SessionOrchestrator {
  ...
  _handleProductChange(productId, now) {
    ...
    // Capture viewed products BEFORE updating state, to detect revisits
    const viewedProducts = this.stateStore.getState().sessionMemory &&
      this.stateStore.getState().sessionMemory.viewedProducts;
    const isRevisit = Array.isArray(viewedProducts) && viewedProducts.includes(productId);
    ...
    // REVISIT_AFTER outcome: fired when user returns to a previously-viewed product.
    // This was previously unreachable (was inside USER_ACTION handler checking for
    // PRODUCT_CHANGED event type — structural impossibility). Fixed here.
    if (isRevisit && this.outcomeTracker && this._sessionId) {
      try {
  ...
  _handleUserAction(payload, now) {
    ...
    if (type === 'add_to_cart') {
      if (typeof this.fatigueEngine.registerPositiveSignal === 'function') {
        // FIX: registerPositiveSignal expects a destructured object { now }, not a bare number
    ...
    // ── Outcome tracking ─────────────────────────────────────────────────────
    // BUG FIX: Previously referenced `event` (not in scope — ReferenceError in
    // strict mode, caught silently). Outcome tracking was completely broken for
    // all USER_ACTION events. Fixed: derive outcomeType from `type` (= payload.type),
    // which IS in scope. Also: the original logic was gated on `type==='add_to_cart'`
    // but tried to detect checkout/dismiss/click inside — unreachable. Fixed by
    // moving outcome tracking outside the add_to_cart gate.
    if (this.outcomeTracker && this._sessionId) {
      let outcomeType = null;
      if (type === 'add_to_cart')        outcomeType = OUTCOME_TYPES.ADD_TO_CART_AFTER;
      else if (type === 'checkout' || type === 'purchase') outcomeType = OUTCOME_TYPES.CHECKOUT_AFTER;
      else if (type === 'dismiss_message' || type === 'message_dismiss') outcomeType = OUTCOME_TYPES.DISMISSED;
      else if (type === 'click_message')  outcomeType = OUTCOME_TYPES.CLICKED;
      ...
      if (outcomeType) {
        try {
          ...
          this._reportError(err, 'outcome_tracker_user_action', now);
        }
      }
    }
  ...
  _validateNow(now) {
    ...
    // Negative or zero nowMs corrupts TTL calculations, cooldown windows, and
    // cleanup logic. Reject explicitly to prevent silent state corruption.
    if (now <= 0) {
      throw new RangeError(
        'SessionOrchestrator: `now` must be a positive timestamp (got ' + now + '). ' +
        'Pass Date.now() or a simulated monotonic clock value > 0.'
      );
    }
  }
